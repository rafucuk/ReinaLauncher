...
const { getMojangOS, isLibraryCompatible, mcVersionAtLeast }  = require('reina-core/common')
...
const { createAuthlibManager } = require('./authlibmanager')
...
class ProcessBuilder {
    ...
    async build(){
        ...
        const authlibManager = createAuthlibManager(ConfigManager.getCommonDirectory());
        const authServerURL = this.server.rawServer.authenticationServer || 'https://reinauth.com/api/yggdrasil/authserver';
        let authlibArgs = await authlibManager.getJVMArgs(authServerURL);
        
        // Ensure that `authlibArgs` is an array (wrap it in an array if needed)
        if (!Array.isArray(authlibArgs)) {
            authlibArgs = [authlibArgs];
        }
    ...
    _constructJVMArguments112(mods, tempNativePath){
        ...
        if(process.platform === 'darwin'){
            args.push('-Xdock:name=ReinaLauncher')
